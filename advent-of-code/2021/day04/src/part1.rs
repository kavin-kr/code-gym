...
fn main() {
    ...
    println!("{:?}", first_bingo_score(&number_order, &boards));
}

fn first_bingo_score(number_order: &[u32], boards: &[Vec<Vec<u32>>]) -> u32 {
    ...
    let mut boards = boards
        ...
        .map(|board| {
            board
                .iter()
                .enumerate()
                .flat_map(|(i, items)| items.iter().enumerate().map(move |(j, &val)| (val, (i, j))))
                .collect::<HashMap<_, _>>()
        })
        .collect::<Vec<_>>();
...
mod tests {
    ...
    fn test_first_bingo_score() {
        ...
        assert_eq!(first_bingo_score(&number_order, &boards), 4512);
    }
}
